...
const Deck = props => {
  const {cards , currentState , changer} = props
  ...
  return(
    <div>
      ...
      cards.map(card => 
        {return card.flipped ?
        <img  name={card.id} src={`https://raw.githubusercontent.com/C4Q/AC_4_Web/master/units/react/exercises/objects_and_arrays/cards/${card.name}.png`} />
          : <img name={card.id} onClick={changer} src="https://raw.githubusercontent.com/C4Q/AC_4_Web/master/units/react/exercises/objects_and_arrays/cards/back.png" />
        }
      )}
   </div>
  )
